...
from autogpt_p.evaluation.planner_evaluation_config import PlannerEvaluationConfig, ModelEnum, AutoregressionEnum, \
...
from autogpt_p.evaluation.planner_evaluation_scenario import PlannerEvaluationScenario
...
class PlannerEvaluation:
    ...
    def _read_scenarios(self, scenario_file) -> List[PlannerEvaluationScenario]:
        with open(scenario_file, 'r') as file:
            ...
            for row in reader:
                ...
                print("Reconstructed" + str(goal2))
                print("Collapsed Goal " + str(goal).strip("()"))
                print(str(goal2) == str(goal))
                min_costs = row['min_costs']
                scenarios.append(
                    PlannerEvaluationScenario(SimulatedScene(objects, relations, locations), task, goal, min_costs))
    ...
    def evaluate(self):
        ...
        average_cost_rate_success = df.loc[df['success'], 'cost_rate'].mean()
    ...
    @classmethod
    def from_config(cls, config: PlannerEvaluationConfig) -> PlannerEvaluation:
        ...
        planner = AutoGPTPlanner(llm, [], [], [], oamdb, actor_skill_mapping=actor_skill_mapping, max_loops=max_loops, save_pddl_files=False)

        return PlannerEvaluation(planner, actor_skill_mapping, os.path.join(base_path, scenario))
...
if __name__ == "__main__":
    ...
    configs = [PlannerEvaluationConfig(ModelEnum.GPT_5, AutoregressionEnum.ON,
                                     ClassesEnum.SAYCAN, ScenarioEnum.SAYCAN_LONG_HORIZON)]
    for config in configs:
        file_name = FILE_FORMAT.format(config.model, config.autoregressive, config.classes, config.scenario)
        logging.basicConfig(filename=os.path.join(PLANNING_LOGS_DIR, file_name + ".log"),
                            level=logging.INFO,
                            format='%(message)s')
        results_file_path = os.path.join(PLANNING_RESULTS_DIR, file_name + ".csv")

        evaluation = PlannerEvaluation.from_config(config)
