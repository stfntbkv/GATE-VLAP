#!/usr/bin/env python3
"""
Test script for LIBERO_OBJECT BDDL conversion to AutoGPT+P format
"""
...
def test_libero_object_conversion():
    """Test the LIBERO_OBJECT BDDL to AutoGPT+P conversion"""
    ...
    print("🚀 Starting LIBERO_OBJECT BDDL Conversion Test")
    ...
    test_bddl_file = os.path.join(libero_bddl_dir, "libero_object", "pick_up_the_alphabet_soup_and_place_it_in_the_basket.bddl")
    ...
    # Test converting the libero_object suite
    print("\n📦 Converting LIBERO_OBJECT suite...")
    try:
        csv_file = converter.convert_suite_to_csv("libero_object")
        print(f"✅ Successfully converted libero_object suite")
...
def check_libero_object_dependencies():
    """Check if all required dependencies for LIBERO_OBJECT are available"""
    print("🔍 Checking LIBERO_OBJECT dependencies...")
    ...
    try:
        ...
        # Check if LIBERO_OBJECT BDDL files exist
        bddl_dir = os.path.join(libero_dir, "libero/libero/bddl_files/libero_object")
        if not os.path.exists(bddl_dir):
            print(f"❌ LIBERO_OBJECT BDDL directory not found: {bddl_dir}")
            return False
        
        # Count total BDDL files in libero_object
        bddl_files = [f for f in os.listdir(bddl_dir) if f.endswith('.bddl')]
        print(f"📊 Found {len(bddl_files)} BDDL files in libero_object")
        ...
        print("✅ All LIBERO_OBJECT dependencies check passed")
        return True
...
if __name__ == "__main__":
    print("🧪 LIBERO_OBJECT-to-AutoGPT+P Conversion Test")
    print("=" * 50)
    
    # Check dependencies first
    if not check_libero_object_dependencies():
        print("❌ Dependency check failed. Please ensure LIBERO_OBJECT is properly set up.")
        sys.exit(1)
    
    # Run conversion test
    success = test_libero_object_conversion()
    
    if success:
        print("\n🎉 LIBERO_OBJECT conversion test completed successfully!")
        print("You can now run the full LIBERO_OBJECT evaluation using libero_planner_evaluation.py")
    else:
        print("\n💥 LIBERO_OBJECT conversion test failed!")
